...
contract FlightSuretyApp {
    ...
    uint256 private constant AIRLINE_BASE_NUMBER = 4;
    ...
    */ 
    ...
    function registerAirline(address airline, string airlineName) external requireIsOperational {
        if(numberOfRegisteredAirlines <= AIRLINE_BASE_NUMBER) {
            ...
            if(multiCalls.length >= numberOfRegisteredAirlines.div(2).add(1)) {
                flightSuretyData.registerAirline(airline, airlineName);
    ...
    *
    */  
    function registerFlight(string flightNumber, address airline) external {
        bool isRegistered = true;
        uint8 statusCode = STATUS_CODE_UNKNOWN;
        uint256 updatedTimestamp = block.timestamp;        
        
        bytes32 key = getFlightKey(airline, flightNumber, updatedTimestamp);
        Flights[key] = Flight(flightNumber, isRegistered, statusCode, updatedTimestamp, airline);

        emit flightRegistered(flightNumber, airline);
    }
